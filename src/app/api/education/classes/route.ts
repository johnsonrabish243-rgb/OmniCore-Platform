...
export async function GET() {
  ...
  const { data: classes } = await supabase.from("classes").select("*").in("organization_id", orgIds).order("name");
...
export async function POST(request: Request) {
  ...
  const { data: classItem } = await supabase.from("classes").insert({ ...body }).select().single();
